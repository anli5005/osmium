...
-- Modified to work with ComputerCraft

-- SHA-256 code in Lua 5.2; based on the pseudo-code from
-- Wikipedia (http://en.wikipedia.org/wiki/SHA-2)

-- local bxor, rshift, bnot =
--   bit.bxor, bit.brshift, bit.bnot
--

local rshift, bnot =
  bit32.rshift, bit32.bnot

function band(n, bits)
  if bits or bits == 0 then
    return bit32.band(n, bits)
  else
    return n % (2 ^ 32)
  end
end
...
function rrotate(n, bits)
  return rshift(n, bits) + bit32.lshift(n, 32 - bits)
end
